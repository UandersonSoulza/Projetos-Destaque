...
public class ProdutoDao {
    ...
    public boolean save(Produto produto, String userLogin) {
        try (Connection connection = ConexaoBancoDados.getConnection();
            ...
            LogMovimentacao log = new LogMovimentacao(produto.getId(), produto.getName(), "INSERT", produto.getQuantity(), userLogin);
            new LogMovimentacaoDao().save(log);
            return true;
        } catch (Exception e) {
    ...
    public boolean update(Produto produto, String userLogin) {
        try (Connection connection = ConexaoBancoDados.getConnection();
            ...
            if (result) {
                LogMovimentacao log = new LogMovimentacao(produto.getId(), produto.getName(), "UPDATE", produto.getQuantity(), userLogin);
                new LogMovimentacaoDao().save(log);
            }
            return result;
        } catch (Exception e) {
    ...
    public boolean delete(int id, String userLogin) {
        ...
        try {
            LogMovimentacao log = new LogMovimentacao(p.getId(), p.getName(), "DELETE", p.getQuantity(), userLogin);
            new LogMovimentacaoDao().save(log);
        } catch (Exception ex) {
            throw new RuntimeException("Erro ao registrar movimentação de exclusão", ex);
        }
        try (Connection connection = ConexaoBancoDados.getConnection();
